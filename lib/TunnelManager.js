...
class TunnelManager {
  ...
  removeExpiredTunnels (maxAge) {
    ...
    for (const [internetPort, tunnel] of this.tunnels.entries()) {
	  ...
	  console.log(`Tunnel ${internetPort} age: ${beaconAgeInSeconds} seconds`);
      const expired = beaconAgeInSeconds > maxAge
      debug(` - `, { internetPort, beaconAgeInSeconds, expired })
      if (!expired) { continue }
      tunnel.relay.end()
      this.tunnels.delete(internetPort)
	  console.log(`Tunnel ${internetPort} expired`);
