...
class Server {
  ...
  async create () {
    ...
    router.all('/', ctx => {
      ctx.body = 'Hypertunnel Server Port';
    })
    ...
    router.get('/status/:internetPort', async (ctx, next) => {
      ...
      ctx.body = {
		...
		beaconAge: ((new Date()).getTime() - tunnel.lastBeacon.getTime()) / 1000
      }
    })
	
    router.get('/beacon/:internetPort', async (ctx, next) => {
      debug('/beacon', ctx.params.internetPort)
      var tunnel = this.manager.tunnels.get(parseInt(ctx.params.internetPort))
      if (!tunnel) { return ctx.throw(400, `Tunnel ${ctx.params.internetPort} not found`) }
	  tunnel.lastBeacon = new Date()
      ctx.body = {
        beacon: true,
      }
    })
    ...
    router.post('/create', async (ctx, next) => {
      ...
      try {
		...
		console.log(`Tunnel Port ${tunnel.relay.internetPort} created`);
      } catch (err) {
        console.log(err)
    ...
    router.post('/delete', async (ctx, next) => {
      ...
      try {
		...
		console.log(`Tunnel Port ${body.internetPort} deleted`);
      } catch (err) {
        console.log(err)
